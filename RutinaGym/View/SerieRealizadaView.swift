        ...
        VStack {
            ...
            if let resumenItems = entrenamientoRealizado.entrenamientoPlanificado?.seriesResumen {
                VStack(alignment: .leading, spacing: 5) {
                    ...
                    ForEach(resumenItems) { item in
                        Button(action: {
                            viewModel.updateSeriesPlanificadas(for: item)
                        }) {
                            HStack {
                                Text(item.resumen)
                                    .font(.subheadline)
                                    .foregroundColor(viewModel.selectedResumenItem?.id == item.id ? .blue : .primary)
                                if viewModel.resumenItemsCompletados[item.id] == true {
                        ...
                        .disabled(viewModel.resumenItemsCompletados[item.id] == true)
            ...
            if viewModel.entrenamientoCompletado {
                Text("¡Entrenamiento completado!")
                    ...
                    .foregroundColor(.green)
                    .padding()
            } else if let selectedItem = viewModel.selectedResumenItem {
                
                if viewModel.resumenItemsCompletados[selectedItem.id] == true {
                        Text("¡Elemento del resumen completado!")
                            .font(.largeTitle)
                            .foregroundColor(.green)
                            .padding()
                } else {
                    ...
                    if !seriesOrdenadas.isEmpty {
                        ...
                        .onChange(of: viewModel.currentIndex) {
                            Task {
                                if viewModel.currentIndex <= seriesOrdenadas.count - 1 {
                                    ...
                                    viewModel.selectedResumenItem = nil
                                }
